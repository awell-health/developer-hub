...
<Alert type="info" title="Compatibility and API version">
  <p>
    Please note that this endpoint can only be used if you also use the{' '}
    <a href="/awell-score/docs/api-reference/perform-calculation">
      v2 endpoint
    </a>{' '}
    to perform calculations as the v1 endpoint does not return an `id` for the
    calculation result that you need to call this endpoint.
  </p>
</Alert>
