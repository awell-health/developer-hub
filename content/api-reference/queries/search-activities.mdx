---
title: Search activities
description: Search activities by status, type, patient and other criteria
---

> Coming soon!

> We suggest reading the page on our [domain model](/docs/getting-started/domain-model) first to get an understanding of the activity model.

## Query

```graphql
query activities(
  $filters: FilterActivitiesParamsGraphqlType
  $pagination: PaginationParamsGraphqlType
  $sorting: SortingParamsGraphqlType
) {
  activities(filters: $filters, pagination: $pagination, sorting: $sorting) {
    activities {
      id
      stream_id
      date
      subject {
        id
        name
        type
      }
      action
      object {
        id
        name
        type
      }
      indirect_object {
        id
        name
        type
      }
      resolution
      status
    }
  }
}
```

## Variables

### Filters

All filter values are optional, if you don't provide any filter all activities in your tenant will be returned.

All `in` filters accept a list of values and result in an `is any of` search criteria. The `eq` filter accepts a single string and results in `is exact match` search criteria.

When specifying multiple filters, the resulting criteria is the conjuction of all filters.

```json
{
  "filters": {
    "action": {
      "in": []
    },
    "activity_status": {
      "in": []
    },
    "activity_type": {
      "in": []
    },
    "pathway_definition_id": {
      "in": []
    },
    "patient_id": {
      "eq": ""
    }
  }
}
```

### Pagination and sorting

Pagination and sorting are optional, **though we recommend not implementing this query without pagination**. Not using pagination means that the API will return all activities at once which might cause performance issues.

```json
{
  "filters": {},
  "pagination": {
    "offset": 0,
    "count": 20
  },
  "sorting": {
    "sort_field": "date",
    "sort_direction": "desc"
  }
}
```
