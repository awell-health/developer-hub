...
2. **Callbacks:** they allow you to communicate with the Design (preview) or Orchestration engine and delegate back control to them. An extension either needs to call `onComplete` or `onError` to complete the corresponding activity.
