...
1. Make sure that you're calling the correct [endpoint](/api-reference/overview/endpoints) and have an [API key to authorize your requests](/api-reference/overview/authorization)
2. Fire up a demo app, use our [Playground](/api-reference/overview/playground), or [Postman collection](https://documenter.getpostman.com/view/19166134/UVXnGEJb) to follow along
...
This query returns all published pathway definitions in your tenant so you need to do a find on `id` to get the baseline data points of the pathway you're interested in (you'll need the [`pathway_definition_id`](/docs/getting-started/concepts-and-terminology#pathway-definition-id) to do the find on the `id ` field).
