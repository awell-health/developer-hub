...
Given that they are completely separate environments, there is no synchronization between them. If you have transferred a flow to production before but apply edits on sandbox, then you'll have to request promotion again.
...
There's no UI or feature in Awell Studio to promote care flows from Sandbox to Production yourself. However, **we don't want you to manually rebuild your care flows** so our team has internal tooling to easily transfer your flows between environments. You can request care flow promotion by clicking the button below.
...
When you request a promotion of your care flow from Sandbox to Production, we'll create a snapshot of the current state (i.e. the latest published version) of your flow and move that to your production tenant.

**Please note that your care flow history is not transferred**, but only a snapshot of the current state of your flow.

### Promotion should be unidirectionally

**Promoting care flows should be unidirectionally and we advise to only promote from Sandbox to Production.** However, we can support you with doing the inverse and promote a version of your care flow in Production to Sandbox.

If, for compliance reasons, you prefer doing edits to your care flows in Sandbox first, we advise you to only design in Sandbox and to not do any edits to your care flows in the Production environment.

### Requesting promotion for an already promoted pathway

There are 2 possible scenarios when requesting promotion of a care flow from Sandbox to Production:

1. The care flow doesn't exist yet in Production, i.e. first time promotion
2. The care flow already exists in Production, i.e. the care flow was already promoted before

**If the care flow doesn't exist yet in Production**, we'll make sure to create it and immediately publish a new version (in this case version 1) that represents the latest published version of the care flow in the Sandbox environment.

**If the care flow already exists in Production,** we'll publish a new version into the already existing care flow that represents the latest published version of the care flow in the Sandbox environment.
