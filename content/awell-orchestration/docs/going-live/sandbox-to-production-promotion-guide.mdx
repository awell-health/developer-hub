...
Given that Sandbox and Production are completely separate environments, there is no synchronization between them. If you have promoted a care flow to production before but apply edits on sandbox, then you'll have to request promotion again.
...
When you request a promotion of your care flow from Sandbox to Production, we'll create a snapshot of the current state (=working version) of your flow and move that to your production tenant.
...
If, f.e. for compliance reasons, you prefer doing edits to your care flows in Sandbox first, we advise you to only design in Sandbox and to not do any edits to your care flows in the Production environment. When you design or edit care flows in both environments, you might lose changes you have done when care flows are promoted.
