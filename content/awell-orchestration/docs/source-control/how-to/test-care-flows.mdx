...
[Because your care flow is stored as JSON](/awell-orchestration/docs/source-control/concepts/components), we can easily perform assertions, such as checking for specific values or conditions within your JSON files.
...
- We will run our tests with Jest by using a GitHub action

Please note that you are free to use any other testing framework or language and are not restricted to Jest/JavaScript.
...
or simply copy and paste the example file below
...
**That's it!** We have successfully build and executed our first test.
...
By levarging GitHub Actions, we can create a workflow that automatically runs all of our tests when a new version of our care flow is published (=when there is a commit to the `main` branch).
...
**Make sure to push all of these changes to your repository.** Next time when you publish your care flow and a commit is made to your repository, you will see that your test suite will run automatically!
