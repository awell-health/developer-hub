...
The payload structure of each event differs based on the type of event. Every event does have a `type` field that you must check. In our [API reference](/awell-orchestration/api-reference/webhooks/pathway-started) under `webhooks` you can find the exact payloads for all event types. Based on the event type, your endpoint can parse the payload of each event.
