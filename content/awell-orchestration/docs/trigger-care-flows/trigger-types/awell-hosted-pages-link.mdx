...
<Alert title="Sharing PHI with Awell" type="info">
  <p>
    Please note that you have full control over how much PHI or demographic data
    you choose to share with Awell. We are equipped to manage care flows even
    for patients who remain anonymous.{' '}
    <strong>
      The specific details you need to provide largely depend on the design of
      your care flow.
    </strong>{' '}
    <br />
    <br />
    For example, if your care flow incorporates conditional logic based on the
    patient's sex and birth date, then these details must be shared with (or
    captured by) Awell. Additionally, we advise including at least one
    identifier to ensure that a patient profile in Awell can be matched with a
    patient in your own system.
  </p>
</Alert>
...
```URL
Example URL     https://goto.awell.health/c/86eay?identifier=<identifier_system>|<identifier_value>
Query string    ?identifier=<identifier_system>|<identifier_value>
```
...
<Alert title="Query parameters" type="warning">
  <p>
    We only allow identifiers in URL query parameters for security reasons;
    passing any other patient data (PHI) is not permitted. Consequently, when
    using the shareable link, you are limited to passing only an identifier.
    This limitation is particularly relevant when a patient profile does not yet
    exist in Awell, as no additional patient data can be passed through the
    link. Moreover, passing baseline is also not supported via the shareable
    link.
  </p>
</Alert>
  ...
  <table>
    <thead>
      <tr>
        ...
        <th className="text-center">Supports passing patient data</th>
      </tr>
    </thead>
    <tbody>
      <tr>
      ...
      <tr>
        ...
        <td>
          <svg
            ...
            fill="currentColor"
            aria-hidden="true"
            class="h-5 w-5 text-slate-400 mx-auto"
          >
            <path
              fill-rule="evenodd"
...
### Identification during the care flow

In addition to using the shareable Hosted Pages link and Sessions, another option is to identify a patient within your care flow and assign the care flow to a patient profile or update the current patient profile if relevant. This approach involves creating specific actions within your care flow designed to support patient identification, update, and assignment.

**Here is a list of situations where this applies and the typical action(s) you would want to take in response.**

<div className="table-wrapper">
  <table>
    <thead>
      <tr>
        <th className="w-2/5">Scenario</th>
    ...
    <tbody>
      <tr>
        <td>
          Completely anonymous patient profile
          <br />
          (no identifier, no PHI)
        </td>
        <td>
          Collect patient info (e.g. patient fills form) and update patient in
          Awell
        </td>
      </tr>
      <tr>
        <td>Patient profile with one or more identifiers</td>
        <td>
          <ul
            style={{
          ...
          >
            <li className="text-base">
              Collect patient info (e.g. patient fills form) and update patient
              in Awell
            </li>
            <li className="text-base">
              Retrieve patient info from a system (based on identifier) and
              update patient in Awell
            </li>
          </ul>
        </td>
      </tr>
      <tr>
        <td>A patient has a duplicate patient profile in Awell</td>
        <td>
          Find existing profile by identifier and assign current care flow to
          existing patient
        </td>
      </tr>
    </tbody>
...
Awell provides builders with a toolbox of actions to support you in building an identification process within your care flow that makes sense for your organization and your use case(s):

- **Form actions** to collect patient demographics from the patient or **extensions and API call actions** to fetch patient demographics from a remote source;
- **Update patient action** to update a patient profile;
- **Find patient by identifier** to find a patient based on the provided identifier;
- **Assign care flow** to assign a care flow to an existing patient;

More information about these actions and how to use them can be found in our [Help Center](https://help.awellhealth.com/en/).
