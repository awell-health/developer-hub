...
**Awell can generate a shareable link for your published care flow that, upon clicking, starts a care flow** and immediately returns a session URL to [Awell Hosted Pages](/awell-orchestration/docs/awell-hosted-pages/overview-hosted-pages) so the user who clicked the link can immediately interact with their activities.
...
2. Since a patient initiates the trigger by clicking a link, there will be an instant feedback mechanism where [Hosted Pages](/awell-orchestration/docs/awell-hosted-pages/overview-hosted-pages) will be displayed to the user, enabling them to immediately complete their activities.
