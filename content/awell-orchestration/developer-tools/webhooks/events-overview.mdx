---
title: Available events
description: Learn about what types of webhooks are available
---

## pathway.started

Triggered every time a pathway is started.

## pathway.completed

Triggered every time a pathway is completed.

## activity.created

Triggered every time an activity in a pathway is created, including system activities.

## activity.completed

Triggered every time an activity in a pathway is completed, including system activities.

## form.submitted

Triggered every time a form is submitted.

## data_point.collected

Triggered every time a new value for a data point is collected.

## reminder.created

Triggered every time a reminder for a pending activity is created.

## session.started

Triggered every time a hosted session is started.

## session.completed

Triggered every time a hosted session is completed.

## session.expired

Triggered every time a hosted session is expired.
