---
title: Start care flow with patient's identifier
description: Start a new care flow for a given patient using patient identifier
---
...
To use identifiers, they need to be first set up on the tenant level. Learn more about how to set up [identifiers](https://developers.awellhealth.com/awell-orchestration/docs/misc/patient-identifiers).
...
You have the ability to pass some initial data when starting a care flow with baseline data points. What data you can pass on care flow start needs to be configured in Awell Studio.
...
<Alert type="info" title="Configure baseline data points">
  <p>
    <a
      ...
      target="_blank"
    >
      Learn how to create and configure baseline data points for your care flows
      in Awell Studio.
    </a>
  </p>
</Alert>

When your baseline data points are configured and your care flow is published, you can query the available baseline data points via the `publishedPathwayDefinitions` query (see documentation [here](/awell-orchestration/api-reference/queries/get-published-pathways)).

> There are `required` and `optional` baseline data points. A care flow will not be started until all required baseline data points have a value.
...
```graphql
mutation StartCareflowWithPatientIdentifier($input: StartCareflowWithPatientIdentifierInput!) {
  startCareflowWithPatientIdentifier(input: $input) {
    careflow_id
    patient_id
  }
}
...
<figure className="w-full flex flex-col justify-center text-center relative">
  <div className="mx-auto rounded-md">
    <a
      ...
      target="_blank"
      title="How to find the care flow definition ID in Awell Studio"
    >
      <img
        src="https://res.cloudinary.com/da7x4rzl4/image/upload/v1704991524/Developer%20portal/pathway_definition_id.png"
        alt="How to find the care flow definition ID in Awell Studio"
      />
    </a>
  </div>
