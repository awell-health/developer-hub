...
When starting a care flow, Awell Orchestration automatically chooses the most recent published version of the specified care flow definition. You can optionally pass a `release_id` to start a specific version.
...
```json
{
  "input": {
    ...
    "release_id": "{{RELEASE_ID}}" // optional, defaults to latest published version
  }
}
```
...
```json
{
  "input": {
    ...
    "pathway_definition_id": "{{PATHWAY_DEFINITION_ID}}",
    "release_id": "{{RELEASE_ID}}", // optional, defaults to latest published version
    "create_patient": true // optional, creates a new patient if the identifier doesn't match an existing one
  }
}
```
