...
Please note that this mutation can only be used to create a Hosted Pages session for a care flow that is already active. Have a look at our [Start pathway and Hosted Pages session mutation](/awell-orchestration/api-reference/mutations/start-care-flow-and-hosted-pages-session). If you would like to start a care flow and immediately create a session.
...
The `pathway_id` specifies which care flow instance there are activities to complete for and the `stakeholder_id` specifies the stakeholder.
