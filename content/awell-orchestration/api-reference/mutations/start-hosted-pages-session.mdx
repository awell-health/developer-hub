...
1. When Awell notifies you via the `activity.created` webhook or through a specific action in the care flow, such as an API call, indicating that a stakeholder needs to complete an activity in a care flow, your application creates a new Hosted Pages Session with this mutation. **This will will give you access to a session URL**.
2. You embed (or redirect staeholder to) Hosted Pages as an iframe in your app so the stakeholder can interact with their pending activities.
