...
Every careflow is linked to a patient in Awell. Patient is uniquely identified in Awell by id or an FHIR like identifier.
Learn more about [identifiers](/awell-orchestration/docs/misc/patient-identifiers).

This mutation allows you to start a careflow for an existing or a new anonymous patient. You can pass in `patient_id` or `patient_identifier` to start a care flow for an existing patient. If both are passed in, only `patient_id` will be used.

If no `patient_id` or `patient_identifier` is provided, we automatically create an anonymous patient.
