...
**Given form responses are polymorphic, the value for a question should always be sent as a string.** Values are validated and deserialized on the Awell side and we will throw an error if a value cannot be converted to the corresponding data point value type.

In the below table you find all question types and some example values for reference.
