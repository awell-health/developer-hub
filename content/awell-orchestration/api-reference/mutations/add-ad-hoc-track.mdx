...
<Alert title="Rate limits on tracks">
  <p>Please be aware that you can only activate one same track in a care flow per minute. Adding multiple tracks at a time will queue them up for activation with a delay.</p>
</Alert>
...
```json
{
  "input": {
    ...
    "track_id": "{{TRACK_ID}}",
    "allow_any_track_definition": false // optional, defaults to false
  }
}
```
  ...
  <table>
    ...
    <tbody>
      <tr>
        <td>pathway_id</td>
        <td>Yes</td>
        <td>The id of the patient care flow to add the track to</td>
        <td>string</td>
      </tr>
      <tr>
        <td>track_id</td>
        <td>Yes</td>
        <td>The id of the ad hoc track to add</td>
        <td>string</td>
      </tr>
      <tr>
        <td>allow_any_track_definition</td>
        <td>No</td>
        <td>When true, allows adding any track definition regardless of its activation trigger configuration</td>
        <td>boolean</td>
      </tr>
    </tbody>
  </table>
