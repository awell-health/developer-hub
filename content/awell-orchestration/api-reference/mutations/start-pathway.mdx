...
By creating a new patient you will get a patient id you can use to start your care flow. Or if you created a patient with a your own FHIR like identifier, you can use that identifier to start a care flow.

To use identifiers, they need to be first set up on the tenant level. Learn more about how to set up [identifiers](/awell-orchestration/docs/misc/patient-identifiers).
...
There are two mutations to start a care flow for a patient, they only differ by how to provide information for which patient to start a care flow, via Awell patient id or via FHIR like identifier.

### Mutation - StartPathway
