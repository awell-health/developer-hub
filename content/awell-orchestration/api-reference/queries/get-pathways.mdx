...
<code className="language-json" highlightedRows={[2, 3, 4, 5, 6, 7, 8]}>
  {`
  {
...
<code className="language-json" highlightedRows={[9, 10, 11, 12]}>
  {`
  {
    "pathway_definition_id": "{{ PATHWAY_DEFINITION_ID }}",
    "patient_id": ["{{ PATIENT_ID_1 }}", "{{ PATIENT_ID_2 }}"],
...
<HowToUse
  stories={[
    {
      title: 'Create a working list with pathways',
      url: '/coming-soon',
      badges: ['Custom integration', 'Pathways', 'Operations'],
      description:
        'Learn how to create a dashboard or list of all pathways that are active for a given pathway definition id, including filtering and pagination.',
    },
  ]}
/>
