...
description: Get all the form definitions in a published care flow
---

This query allows us to fetch the form details for all form defintions in a published care flow. Want to learn more about how to display the form in your UI and conditionally show/hide the correct questions based on the response? [Click here](/awell-orchestration/api-reference/queries/get-form)!

## All forms vs. forms that will be completed

Please note that this query returns all forms in a published care flow but that this doesn't necessarily mean that all these forms will be completed as that is dependent on the logic built into the care flow.
...
- The query will return all published forms in the care flow so Form A, B, and C but depending on the workflow some forms might never be completed.
...
The `release_id` is optional. If not specified, we return the forms for the latest published version of the care flow.
