---
title: Source Control
description: Synchronize your flows to SCM providers like GitHub.
---

Source Control allows you to sync your care flows as code to remote Source Control Management (SCM) providers, such as GitHub. When Source Control is set up, your care flow is synced to your repository every time you publish a new version of your care flow.

Syncing care flows with SCM tools has many benefits:

1. **Care flows-as-code:** by treating care flow components as code you have unparalleled control and ownership.
2. **Direct integration with CI/CD pipelines:** seamlessly incorporate care flow updates into your existing CI/CD systems.
3. **Code-based testing:** verify the functionality of care flows through your code-testing protocols.
4. **Visibility on changes:** inspect and view changes between different versions of your care flow through code

## Get started

Watch the video or follow a quickstart to get started with Source Control.

<iframe
  className="video"
  src="https://www.youtube.com/embed/mKKOjHreut8?si=1H1RlAp0HoAZKbUd"
  style={{ aspectRatio: '16 / 9', width: '100%' }}
  frameBorder="0"
  allow="accelerometer; autoplay; encrypted-media; gyroscope;"
  allowFullScreen
/>

## How-to

- Source control sync works with both GitHub and GitLab (Please get in touch if you use another provider, e.g. BitBucket).
- In both cases, you'll want a Personal Access Token (_not a Project Access Token_ for those working in GitLab).
- In GitHub, it is recommended to use fine-grained tokens (although you can also use classic tokens, which have broader permissions). Create a PAT with `read access to Metadata` and `read/write access to Contents`, and sure to enable permissions for the appropriate repository.
- IN GitLab, [create a Personal Access Token](https://gitlab.com/-/user_settings/personal_access_tokens) with read_repo and write_repo permissions
