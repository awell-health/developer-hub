...
| 385950590, 385950591, 385950593 (leaked_secret, medium 50) curl header token | `requests-and-responses.mdx` (two snippets), `authorization.mdx` | #234's `-H "apiKey: $AWELL_API_KEY"` did **not** clear it on the 2026-09-10 rescan — the rule fires on any `curl … -H "apiKey: …"` shape, variable or literal. Snippets now read `-H "apiKey: ${AWELL_API_KEY:?set AWELL_API_KEY first}"` with a comment line above, which is also better docs (a missing variable fails loudly instead of sending an empty header). The rule does not run locally, so this is the second attempt, not a verified clear; if it survives the next scan the finding is a dashboard close — a docs page has to show the header. |
