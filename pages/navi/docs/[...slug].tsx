  ...
  return (
    <div>
      <SEO
        ...
        url={`/${Space.NAVI}/docs/${slug}`}
        canonicalUrl={`/${Space.NAVI}/docs/${slug}`}
      />
      {heading && (
        <DocsHeader
          heading={heading}
          title={frontMatter.title}
          description={frontMatter.description}
          githubUrl={`/content/${Space.NAVI}/docs/${slug}.mdx`}
          postman={{
            showPostmanButton: Boolean(frontMatter.showPostmanButton),
...
export const getStaticProps: GetStaticProps = async (context) => {
  ...
  const { frontMatter, content } = await getNaviDoc(slugString)
