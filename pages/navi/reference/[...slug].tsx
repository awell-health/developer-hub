  ...
  return (
    <div>
      <SEO
        ...
        url={`/${Space.NAVI}/reference/${slug}`}
        canonicalUrl={`/${Space.NAVI}/reference/${slug}`}
      />
      {heading && (
        <DocsHeader
          heading={heading}
...
export const getStaticProps: GetStaticProps = async (context) => {
  ...
  const { frontMatter, content } = await getNaviDoc(`reference/${slugString}`)
...
export const getStaticPaths: GetStaticPaths = () => {
  const docs = getAllNaviDocs().filter((d) => d.slug.startsWith('reference/'))
  const paths = docs.map((doc) => ({
    params: { slug: doc.slug.replace('reference/', '').split('/') },
  }))
