  ...
  return (
    <div>
      <SEO
        ...
        url={`/${Space.NAVI}/how-to/${slug}`}
        canonicalUrl={`/${Space.NAVI}/how-to/${slug}`}
      />
      {heading && (
        <DocsHeader
          heading={heading}
          title={frontMatter.title}
          description={frontMatter.description}
          githubUrl={`/content/${Space.NAVI}/how-to/${slug}.mdx`}
        />
      )}
...
export const getStaticProps: GetStaticProps = async (context) => {
  ...
  const { frontMatter, content } = await getNaviDoc(`how-to/${slugString}`)
...
export const getStaticPaths: GetStaticPaths = () => {
  const docs = getAllNaviDocs().filter((d) => d.slug.startsWith('how-to/'))
  const paths = docs.map((doc) => ({
    params: { slug: doc.slug.replace('how-to/', '').split('/') },
  }))
